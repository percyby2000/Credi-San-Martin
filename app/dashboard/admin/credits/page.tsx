...
export default function CreditsPage() {
  ...
  const approveRequest = (requestId: number) => {
    ...
    alert("Solicitud de crédito aprobada exitosamente")
  }

  const rejectRequest = (requestId: number, reason = "No especificado") => {
    setCreditRequests((prev) =>
      prev.map((request) =>
        request.id === requestId
          ? {
    ...
    alert("Solicitud de crédito rechazada")
  }

  const sendToReview = (requestId: number) => {
    setCreditRequests((prev) =>
      prev.map((request) => (request.id === requestId ? { ...request, status: "review" } : request)),
    )
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center space-x-2 mb-6">
          ...
          <span className="text-gray-900 font-medium">Créditos y Préstamos</span>
        </div>

        {/* Page Header */}
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Créditos y Préstamos</h1>
          ...
          <div className="flex space-x-3">
            ...
            <Button onClick={() => setShowNewCreditForm(true)} className="bg-green-500 hover:bg-green-600">
                ...
                {request.status === "approved" && request.approvedBy && (
                  <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-sm text-green-700">
                      <strong>Aprobado por:</strong> {request.approvedBy} el {request.approvedDate}
                    </p>
                  </div>
                )}
                    ...
                    {request.status === "pending" && (
                      <>
                        <Button
                          ...
                          onClick={() => sendToReview(request.id)}
                        >
